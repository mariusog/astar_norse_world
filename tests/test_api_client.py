...
class TestListRounds:
    def test_returns_parsed_json(self, client: AstarClient) -> None:
    ...
    def test_calls_correct_endpoint(self, client: AstarClient) -> None:
        resp = _mock_response(json_data=[])
        with patch.object(client._session, "request", return_value=resp) as mock_req:
            client.list_rounds()
        mock_req.assert_called_once_with("GET", "https://test.api/astar-island/rounds")


class TestGetRound:
    def test_returns_detail(self, client: AstarClient) -> None:
...
class TestQuery:
    def test_sends_correct_body(self, client: AstarClient) -> None:
    ...
    def test_increments_budget(self, client: AstarClient) -> None:
        resp = _mock_response(json_data={"grid": [[0]]})
        with patch.object(client._session, "request", return_value=resp):
    ...
    def test_budget_warning_at_threshold(self, client: AstarClient) -> None:
    ...
    def test_budget_exhausted_raises(self, client: AstarClient) -> None:
...
class TestSubmit:
    def test_applies_probability_floor(self, client: AstarClient) -> None:
        pred = np.zeros((3, 3, 6))
        pred[:, :, 0] = 1.0
    ...
    def test_preserves_shape_in_body(self, client: AstarClient) -> None:
...
class TestNewEndpoints:
    ...
    def test_get_active_round_found(self, client: AstarClient) -> None:
        rounds = [{"id": "r1", "status": "completed"}, {"id": "r2", "status": "active"}]
        resp = _mock_response(json_data=rounds)
        client._session.request = MagicMock(return_value=resp)
    ...
    def test_get_active_round_none(self, client: AstarClient) -> None:
        resp = _mock_response(json_data=[{"id": "r1", "status": "completed"}])
        client._session.request = MagicMock(return_value=resp)
        assert client.get_active_round() is None
